...
// Project name
set('application', 'workerman-webrtc-prod');
...
task('deploy1', function(){
    ...
    file_put_contents('./docker/k8s.deployment-prod.yaml', parse(file_get_contents('./docker/k8s.deployment-prod.tpl.yaml')));
    run('mkdir -p /log/{{application}}.k8sv2.wpjs.cc');
    run('mkdir -p ~/{{application}}');
    upload('./docker', '~/{{application}}');
    run('/opt/kube/bin/kubectl apply -f ~/{{application}}/docker/k8s.deployment-prod.yaml');
